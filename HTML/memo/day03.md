...
```
    ...
    포함해 모든 사요자들이 동등하게 접근할 수 있도록 컨텐츠를 제작하는 기법
```
...
웹 표준
```
    여러 브라우저에서 동일한 사용자 경험을 줄 수 있도록 하는 기술, 방법등을 의미 크로스 브라우징
```
...
```
    ...
    <strong>
        텍스틀르 굵게 표현하고 싶을 때 사용
        <b> 태그와 동일한 기능
        웹 접근성에 사용
    <em>
        텍스트를 이탤리체로 표현하고 싶을 때 사용
        <i> 태그와 동일한 기능
        웹 접근성에 사용
    <mark>
        텍스트에 하이라힝팅(형광) 효과를 적용
    <del>
        텍스트 중앙에 가로줄을 만들어 텍수트를 지원것과 같은 효과를 줌
    <ins>
        텍스트 밑에 선을 그어 텍스트의 강조효과를 줌
    <sup>,<sub>
        <sup> 태그는 위청자를 표현할 때 사용
        <sub> 태그는 아래청자를 표현할 때 사용
```
...
```
    ...
    <html lang="en">
            ...
            스크린 리더에서 언어를 인식하여 자동으로 음성을 변환하거니,
            해당 언어에 적합한 발음을 제공
            한국어로 설정하려면 lang="ko"
```
...
```
    ...
    --      -----      ------
    요소    속성명      속성값
    속성에 대한 값은 따옴표를 꼭 사용하지 않안도 되지만,
    속성값에 띄어쓰기가 있을 경우 문제가 발생할 수 있어서
    따옴표를 쓰기 권장함(쌍따옴표, 따옴표 둘다 가능)
    ...
    <meta charset="UTF-8">
         ...
         우리나라 윈도우 기본 문자셋인 EUC-KR을 사용해더 되지만,  
         다국어로 인코딩 하기 위해서는 UTF-8 문자셋을 사용하기 권장한다.

    meta name="viewport" content="width=device-width, initial-scale=1.0">
        viewport : 가상의 화면
        모바일로 최적화된 사이트에 포함된 META 속성 설정
```
...
```
    ...
    백터 이미지
            수학적 정보의 혈태들이 만들어내는 결과물
            이미지가 가지고 있는 점, 선, 면의 위치, 색상정보 등을 가지고 있음
            확대 및 축소를 이미지가 깨지지 않음
            일러스트 같은 툴로 편잡
```
...
```
    ...
    상대 경로 
        ...
        이미지가 상위 폴더의 하위 폴더에 있는 경우
                <img src="../하위 폴더명/파일명"> , <img src="./../하위폴더명/파일명>
```